...
import { MapPin, Calendar, Clock, Search, Navigation, Loader2, Thermometer, Wind, Droplets, CloudRain, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const fetchWeatherData = useCallback(async (loc: Location, date: string, hour: number) => {
    ...
    try {
      const response = await fetch(
        `https://archive-api.open-meteo.com/v1/archive?latitude=${loc.lat}&longitude=${loc.lng}&start_date=${date}&end_date=${date}&hourly=temperature_2m,relative_humidity_2m,precipitation,weather_code,wind_speed_10m&timezone=auto`
      );
      ...
      setWeatherData({
        ...
        relativeHumidity: humidities[hourIndex],
        windSpeed: windSpeeds[hourIndex],
        precipitation: precipitations[hourIndex],
