...
const WeatherDisplay: React.FC<WeatherDisplayProps> = ({ data, loading, error, unit }) => {
  ...
  const IconComponent = ((LucideIcons as unknown) as Record<string, React.ComponentType<{ className?: string }>>)[icon] || LucideIcons.Cloud;
      ...
      <div className="space-y-4">
        ...
        <div className="flex overflow-x-auto pb-4 -mx-2 scrollbar-hide space-x-4">
          {data.hourly.time.map((time, index) => {
            ...
            const HourlyIcon = ((LucideIcons as unknown) as Record<string, React.ComponentType<{ className?: string }>>)[hourlyIcon] || LucideIcons.Cloud;
